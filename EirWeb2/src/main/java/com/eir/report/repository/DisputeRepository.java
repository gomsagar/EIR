...
public interface DisputeRepository extends JpaRepository<Dispute, Integer> {
	...
	@Query(value = "select count(*) from dispute d where d.user_id =:userID", nativeQuery=true)
	Integer findDisputeCountByUserId(@Param("userID") Integer userId);
	
	@Query(value = "select count(*) from dispute d where d.user_id=:userID and d.status_id=:statusId", nativeQuery=true)
