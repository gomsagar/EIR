...
public interface ConsumerRequetRepository extends JpaRepository<ConsumerRequest, Integer> {
	...
	@Query(value = "select con.REQUEST_ID, s.STATUS_DESCRIPTION from CONSUMER_REQUEST con inner join REQUEST r on r.REQUEST_ID = con.REQUEST_ID "
			+"inner join STATUS s on con.STATUS_ID = s.STATUS_ID where r.CREATE_DATE >= :fromDate and r.CREATE_DATE <= :toDate",
			nativeQuery=true)
	public List<Object[]> getConsumerRequestByDate(@Param("fromDate") String fromDate,@Param("toDate") String toDate);
	
	@Query(value = "select con.REQUEST_ID, s.STATUS_DESCRIPTION from CONSUMER_REQUEST con inner join REQUEST r on r.REQUEST_ID = con.REQUEST_ID "
			+"inner join STATUS s on con.STATUS_ID = s.STATUS_ID where r.CREATE_DATE >= :fromDate and r.CREATE_DATE <= :toDate and con.REQUEST_ID =:requestID",
			nativeQuery=true)
