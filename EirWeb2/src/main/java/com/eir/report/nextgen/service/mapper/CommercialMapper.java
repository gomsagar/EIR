...
import com.experian.nextgen.ind.model.commercial.uofpojo.Nginquiry;
import com.experian.nextgen.ind.model.commercial.uofpojo.Psummary;
...
public class CommercialMapper {
	...
	public CommercialReportDetails mapData(ResponseInfo responseInfo1) throws IOException
	{
		...
		Template template = velocityEngine
			...
			if(responseInfo.getBusinessResponse() != null && responseInfo.getBusinessResponse().getCOMMCRED() != null
					&& !responseInfo.getBusinessResponse().getCOMMCRED().isEmpty())
			{
			...
			if(responseInfo.getBusinessResponse() != null &&  responseInfo.getBusinessResponse().getNGINQUIRY() != null)
			{
	...
	private CurrentApplication getCurrentApplication()
	{
		...
		if(responseInfo.getBusinessResponse() != null &&  responseInfo.getBusinessResponse().getNGINQUIRY() != null)
		{
			Nginquiry nginquiry = responseInfo.getBusinessResponse().getNGINQUIRY();
			currentApplication.setInqCompanyName(nginquiry.getInqCompanyName());
			currentApplication.setEnquiryPurposeCd(nginquiry.getInqPurposeCd());
			currentApplication.setEnquiryAmount(nginquiry.getInqAmt());
			currentApplication.setInqPan(nginquiry.getInqPAN());
			currentApplication.setLoanAccNo(nginquiry.getInqCompanyBankAccNumber());
			if(nginquiry.getNgInqAddr() != null 
					&& !nginquiry.getNgInqAddr().isEmpty())
			{
				NgInqAddr ngInqAddr = nginquiry.getNgInqAddr().get(0);
...
//	TelephoneInputData telephoneData=mapToTelephoneInputData(responseInfo.getBusinessResponse().getNGINQUIRY().getNgInqTel());
	...
	private ReportSummary getReportSummary() 
	{
		...
		if(responseInfo.getBusinessResponse() != null && responseInfo.getBusinessResponse().getPSUMMARY() != null)
		{
			Psummary psummary = responseInfo.getBusinessResponse().getPSUMMARY();
			//responseInfo.getBusinessResponse().
			if(psummary.getAccsum() != null )
			{
				reportSummary.setNumOfAllAccounts(psummary.getAccsum().getTotalCreditAccount());
				reportSummary.setNumOfActiveAccounts(psummary.getAccsum().getTotalActiveAccounts());
			}
			if(psummary.getGrantor() != null)
			{
				reportSummary.setTotalCreditProviders(psummary.getGrantor().getTotalCreditProviders());
				reportSummary.setCurrentCreditProviders(psummary.getGrantor().getCurrentCreditProviders());	
			}
		
			List<Credtype> credtypelist = psummary.getCredtype();
	...
	private AccountSummary getAccountSummaryFields()
	{
		...
		if(responseInfo.getBusinessResponse() != null && responseInfo.getBusinessResponse().getPSUMMARY() != null)
		{
			...
			List<Currcycnct> currcycncts = psummary.getCurrcycnct();
	...
	public List<RelationshipDetails> getRelationshipDetails() 
	{
		...
		if(responseInfo.getBusinessResponse() != null && responseInfo.getBusinessResponse().getRELATIONS() != null 
				&& !responseInfo.getBusinessResponse().getRELATIONS().isEmpty())
		{
	...
	private List<EnquiryDetails> getEnquiryDetails()
	{
		...
		if(responseInfo.getBusinessResponse() != null )
		{
